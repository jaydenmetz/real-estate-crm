...
/**
 * AIManagerPage - Full AI Manager hero page for listings
 *
 * Page 2 of the listings hero carousel.
 * Shows AI monitoring stats, quick action items, and configuration options.
 *
 * Data is lazily loaded from /v1/listings/manager when the page becomes visible.
 */
const AIManagerPage = ({ isVisible = true, onConfigureAI, onViewActivity }) => {
  ...
  const quickItems = [
    {
      icon: '📉',
      text: 'price reductions needed',
      value: quickItemsData.price_reductions,
      type: 'urgent',
    },
    {
      icon: '💬',
      text: 'new inquiries',
      value: quickItemsData.new_inquiries,
      type: 'pending',
    },
    {
      icon: '🏠',
      text: 'showings scheduled',
      value: quickItemsData.showings_scheduled,
      type: 'info',
    },
    {
      icon: '📷',
      text: 'listings need photos',
      value: quickItemsData.photos_needed,
      type: 'success',
    },
  ];
        ...
        <Grid item xs={6} sm={3}>
          {isLoading ? (
            ...
            <AIManagerStatCard
              ...
              valueColor={stats.expiring_soon > 0 ? '#ffd54f' : undefined}
            />
          )}
        </Grid>
        <Grid item xs={6} sm={3}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography sx={{ color: 'rgba(255,255,255,0.9)', fontSize: '0.9rem' }}>
            {aiStatus.monitoring_all
              ? 'Currently monitoring all listings'
