...
/**
 * AIManagerPage - Full AI Manager hero page for appointments
 *
 * Page 2 of the appointments hero carousel.
 * Shows AI monitoring stats, quick action items, and configuration options.
 *
 * Data is lazily loaded from /v1/appointments/manager when the page becomes visible.
 */
  ...
  const quickItems = [
    {
      icon: '❓',
      text: 'unconfirmed',
      value: quickItemsData.unconfirmed,
      type: 'urgent',
    },
    {
      icon: '🔔',
      text: 'need reminders',
      value: quickItemsData.needs_reminder,
      type: 'pending',
    },
    {
      icon: '⚠️',
      text: 'schedule conflicts',
      value: quickItemsData.conflicts,
      type: 'info',
    },
    {
      icon: '✅',
      text: 'completed today',
      value: quickItemsData.completed_today,
      type: 'success',
    },
  ];
        ...
        <Grid item xs={6} sm={3}>
          {isLoading ? (
            ...
            <AIManagerStatCard
              ...
              subtext="Upcoming"
              icon="⏰"
              highlight={stats.upcoming_24h > 0}
              valueColor={stats.upcoming_24h > 0 ? '#ffd54f' : undefined}
            />
          )}
        </Grid>
        <Grid item xs={6} sm={3}>
          {isLoading ? (
            ...
            <AIManagerStatCard
              label="NEED CONFIRM"
              value={stats.confirmations_needed}
              subtext="Awaiting"
              icon="❓"
            />
          )}
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography sx={{ color: 'rgba(255,255,255,0.9)', fontSize: '0.9rem' }}>
            {aiStatus.monitoring_all
              ? 'Currently monitoring all appointments'
