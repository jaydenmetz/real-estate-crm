...
const LeadsHealthDashboard = () => {
  ...
  const [testLeadId, setTestEscrowId] = useState(null);
  const [testLeadIds, setTestEscrowIds] = useState([]);
  ...
  const runAllTests = useCallback(async (forceNewKey = true) => {
    ...
    try {
      ...
      const tests = await healthService.runLeadsHealthCheck();
      ...
      <Container maxWidth="xl">
        <Paper elevation={3} sx={{ p: 3, mb: 3, backgroundColor: 'rgba(255, 255, 255, 0.98)' }}>
          ...
          <Typography variant="body1" color="textSecondary" mb={3}>
            Comprehensive health checks for the Leads API with dual authentication support
          </Typography>
