...
const EscrowWidgetLarge = ({ escrow, index = 0, loading = false }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* LEFT: Property Image - 400px wide (square, matches height for dramatic impact) */}
        <Box
          sx={{
            width: 400,
            minWidth: 400,
        ...
        {/* RIGHT: Content */}
        <CardContent sx={{ flex: 1, p: 4.5, display: 'flex', flexDirection: 'column' }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 2, mb: 3 }}>
            ...
            >
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                >
                  {showCommission ? (
                    <VisibilityOff sx={{ fontSize: 18, color: '#6366f1' }} />
                  ) : (
            ...
            >
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                >
                  {showGross ? (
                    <VisibilityOff sx={{ fontSize: 18, color: '#a855f7' }} />
                  ) : (
                    <Visibility sx={{ fontSize: 18, color: '#a855f7' }} />
                  )}
                </Box>
              </Box>
              <Typography variant="body1" sx={{ fontWeight: 800, fontSize: '1.05rem', color: '#a855f7', letterSpacing: '-0.5px' }}>
                {showGross ? formatCurrency(grossCommission) : maskCommission(grossCommission)}
              </Typography>
            </Box>
