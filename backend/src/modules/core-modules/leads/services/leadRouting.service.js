...
class LeadRoutingService {
  ...
  async findCandidateAgents(leadData) {
    ...
    return Array.from(candidates).map((agentId) => this.agents.find((agent) => agent.id === agentId)).filter((agent) => agent && this.isAgentAvailable(agent));
  }
  ...
  async selectBestAgent(rankedCandidates) {
    ...
    if (this.routingRules.roundRobin.enabled
      ...
      const topCandidates = rankedCandidates.filter((c) => c.matchScore >= rankedCandidates[0].matchScore - 10);
  ...
  detectTerritory(location) {
    ...
    if (locationLower.includes('north') || locationLower.includes('carlsbad') || locationLower.includes('oceanside')) {
      return 'North County';
    } if (locationLower.includes('south') || locationLower.includes('chula vista') || locationLower.includes('national city')) {
      return 'South County';
    } if (locationLower.includes('east') || locationLower.includes('el cajon') || locationLower.includes('santee')) {
      return 'East County';
    } if (locationLower.includes('west') || locationLower.includes('point loma') || locationLower.includes('ocean beach')) {
      return 'West County';
    } if (locationLower.includes('downtown') || locationLower.includes('hillcrest') || locationLower.includes('mission valley')) {
      return 'Central';
    }
  ...
  async balanceWorkload() {
    try {
      const overloadedAgents = this.agents.filter((agent) => agent.currentActiveLeads / agent.maxActiveLeads > 0.9);

      const underloadedAgents = this.agents.filter((agent) => agent.currentActiveLeads / agent.maxActiveLeads < 0.5);
