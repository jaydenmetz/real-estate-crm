...
async function getStats(req, res) {
  try {
    ...
    const statsQuery = 'SELECT COUNT(DISTINCT e.id) as total, COALESCE(SUM(e.purchase_price::numeric), 0) as total_volume, COALESCE(SUM((' + commissionField + ')::numeric), 0) as total_commission FROM escrows e WHERE ' + conditions.join(' AND ');
    ...
    res.json({
      success: true,
      data: {
        total: { value: parseInt(result.rows[0].total) },
        totalVolume: { value: parseFloat(result.rows[0].total_volume) },
        totalCommission: { value: parseFloat(result.rows[0].total_commission) }
      }
    });
  } catch (error) {
