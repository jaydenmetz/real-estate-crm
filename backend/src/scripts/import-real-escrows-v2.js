...
async function importEscrows() {
  ...
  try {
    ...
    for (let i = 0; i < escrows.length; i++) {
      ...
      const addressParts = escrow.propertyAddress.split(',').map(s => s.trim());
      const street = addressParts[0] || escrow.propertyAddress;
      const city = addressParts[1] || 'San Francisco';
      const stateZip = addressParts[2] || 'CA 94110';
      ...
      // Insert escrow with all JSONB data
      ...
      `, [
        ...
        `${escrow.escrowOfficer.toLowerCase().replace(' ', '.')}@escrow.com`,
        escrow.loanOfficer,
        `${escrow.loanOfficer.toLowerCase().replace(' ', '.')}@lender.com`,
        escrow.nhdCompany,
        ...
        escrow.checklists.admin.avid || false
      ]);
