...
class ListingMock {
  ...
  static async findById(id) {
    ...
    return {
      ...
      // Features breakdown
      features: {
        interior: listing.features.filter(f => 
          ['Kitchen', 'Appliances', 'Floors', 'Master', 'Built-ins', 'Theater', 'Wine'].some(keyword => f.includes(keyword))
        ),
        exterior: listing.features.filter(f => 
          ['Pool', 'Landscaping', 'Views', 'Architecture', 'Guest House', 'Solar'].some(keyword => f.includes(keyword))
        ),
        community: listing.features.filter(f => 
          ['Concierge', 'Fitness', 'Rooftop', 'Security', 'Smart Home', 'EV'].some(keyword => f.includes(keyword))
        )
      },
  ...
  static async update(id, data) {
    try {
      ...
      const listingDate = mockListings[index].listingDate;
